...
const FormBuilder =  (opts, element) =>{
  ...
  opts = h.processOptions(opts);
    ...
    let prepFieldVars = function ($field, isNew = false) {
        ...
        if (!field.key){
            field.key = utils.nameAttr(field);
        }
    ...
    let advFields = values => {
        ...
        const advFieldMap = {
            ...
            datajson: () => boolAttribute('datajson', values, {first: i18n.savejson}),
            ...
            listtype: () => selectAttribute('listtype', values, listtype),
    ...
    let appendNewField = function (values, isNew = true) {
      ...
      let fieldButtons = [
		m('a', null, {
          type: 'edit',
          id: data.lastID + '-edit',
          className: 'toggle-form btn fa fa-expand',
          title: i18n.hide,
        }),
        m('a', null, {
          type: 'remove',
          id: 'del_' + data.lastID,
          className: 'del-button btn fa fa-trash-o delete-confirm',
    ...
    $stage.on('change', '[name="listtype"]', (e: any) => {
        let values = $(e.target).find("option:selected").text();
        let parent = $(e.target).parent().parent().next();
        let formData = utils.escapeHtml(data.formData);
        let savehtml = '<label class="false-label">Dữ liệu hiển thị</label><div class="sortable-options-wrap"><ol class="sortable-options ui-sortable"><li class="ui-sortable-handle"><input type="radio" class="option-selected" value="true" name="select-1516354639025-option" placeholder="" checked="true"><input type="text" class="option-label" value="'+values+'" name="select-1516354639025-option"><input type="text" class="option-value" value="'+values+'" name="select-1516354639025-option"><a class="remove btn" title="Xóa đối tượng">×</a></li></ol><div class="option-actions"></div></div>';
        parent.html(savehtml);
    });
    ...
    $stage.on('click touchstart', '.delete-confirm', e => {
        e.preventDefault();
		var result = confirm("Bạn có chắc chắn muốn xóa đối tượng đã chọn?", "Thông báo");
